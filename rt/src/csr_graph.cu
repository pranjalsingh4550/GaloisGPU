...
void CSRGraph::copy_to_cpu(struct CSRGraph &copygraph) {
  ...
  check_cuda(cudaMemcpy(copygraph.edge_data, edge_data, nedges * sizeof(edge_data_type), cudaMemcpyDeviceToHost));
  check_cuda(cudaMemcpy(copygraph.node_data, node_data, nnodes * sizeof(edge_data_type), cudaMemcpyDeviceToHost));
