...
__global__ void gen_prio_gpu(CSRGraphTex graph, unsigned int * prio, unsigned int x, unsigned int y, unsigned int z, unsigned int w)
{
  unsigned tid = TID_1D;
...
void gen_prio(CSRGraphTex graph, unsigned int * prio)
{
...
__global__ void mark_nodes(CSRGraphTex graph, const unsigned int * __restrict__ prio, WorklistT in_wl, WorklistT out_wl)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
...
__global__ void drop_marked_nodes_and_nbors(CSRGraphTex graph, WorklistT in_wl, WorklistT out_wl)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
